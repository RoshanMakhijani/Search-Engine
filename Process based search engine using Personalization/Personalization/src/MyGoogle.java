...
class MyGoogle extends ProcessResult
{
	...
	void fire(String query, PrintWriter out)throws IOException
	{
		try
		{
			...
			URL url = new URL(
				    "https://ajax.googleapis.com/ajax/services/search/web?v=1.0&"
				    + "q="+query+"&key=ABQIAAAAocJ-KvMi0B_FxpNh7qnRDBQhBt6oGohfqEJBSQzz1VHC9nUx0xRCMrXA-ZboKuhYU4VGreZydDjqwQ&start=0&userip=169.254.99.10");
			//URL url=new URL("http://www.wikihow.com/Special:GoogSearch?cx=008953293426798287586%3Amr-gwotjmbs&cof=FORID%3A10&ie=UTF-8&q=hello&siteurl=www.wikihow.com%2FMain-Page&siteurl=www.wikihow.com%2FMain-Page");	
			URLConnection connection = url.openConnection();
				BufferedOutputStream outStream = new BufferedOutputStream(new
						FileOutputStream("D:\\Vijay\\Java\\eclipseWorkspace\\WebCrawler\\FileToIndex\\WayneRooney"));
				...
				//CC.fuzzyCreator(CC);
				//System.out.print("Before oracle");
				//getOracleConnection.insertRows(CC);
				if(MyGoogle.auth)
				{
				...
				rs.next();
					...
					CC.con=(StringBuffer) ois.readObject();
					CC.tot_doc=ois.readInt();
					CC.optout=ois.readBoolean();
					CC.Size=(Vector)ois.readObject();
					for(int i=0;i<CC.Size.size();i++)
						for(int j=0;j<CC.Size.size();j++)
					CC.K[i][j]=ois.readDouble();*/
					//OutputStream out1=new FileOutputStream("C:\\temp.txt");
